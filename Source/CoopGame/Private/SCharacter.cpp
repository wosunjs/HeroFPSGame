...
void ASCharacter::SwitchWeapons()
{
	if (WeaponId == 1) WeaponId = 0;
	else WeaponId = 1;
	//UE_LOG(LogTemp, Log, TEXT("Weapon Changed: %d"), WeaponId);

	// 在蓝图中选择武器，在C++中销毁之前的武器和生成新的武器
	// 销毁武器
	...
	if (WeaponId)
	{
		CurrentWeapon = GetWorld()->SpawnActor<ASWeapon>(ProjectWeaponClass, FVector::ZeroVector, FRotator::ZeroRotator, SpawnParams);
	}
	else {
		CurrentWeapon = GetWorld()->SpawnActor<ASWeapon>(StarterWeaponClass, FVector::ZeroVector, FRotator::ZeroRotator, SpawnParams);
	}
